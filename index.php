...
$googleCxId = getenv('GOOGLE_CX_ID');
$pexelsApiKey = getenv('PEXELS_API_KEY');


if (empty($googleApiKey)) { $googleApiKey = 'YOUR_GOOGLE_API_KEY'; } // Replace with placeholder or key ONLY for local testing if needed
if (empty($googleCxId)) { $googleCxId = 'YOUR_GOOGLE_CX_ID'; } // Replace with placeholder or key ONLY for local testing if needed
if (empty($pexelsApiKey)) { $pexelsApiKey = 'YOUR_PEXELS_API_KEY'; } // Replace with placeholder or key ONLY for local testing if needed
...
if ($action === 'search' && isset($_GET['query'])) {
    ...
    // Check if keys were loaded (either from env or fallback placeholder)
    // This check ensures that even if fallbacks are placeholders, the script knows keys are missing if env vars are not set on host
    if (empty($googleApiKey) || $googleApiKey === 'YOUR_GOOGLE_API_KEY' || empty($googleCxId) || $googleCxId === 'YOUR_GOOGLE_CX_ID' || empty($pexelsApiKey) || $pexelsApiKey === 'YOUR_PEXELS_API_KEY') {
         http_response_code(500); $output_data['error'] = 'API Keys/IDs are not configured correctly. Set Environment Variables on hosting or check local fallbacks.';
    ...
    function makeApiRequest($url, $headers = []) {
        ...
        curl_setopt($ch, CURLOPT_URL, $url); curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); curl_setopt($ch, CURLOPT_TIMEOUT, 15); curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // For local testing ONLY
    ...
    $google_results = []; $google_total_results = 0; $google_error = null;
    $google_api_url = 'https://www.googleapis.com/customsearch/v1';
    // Use the variable holding the key/ID read from getenv() or fallback
    $google_params = [ 'key' => $googleApiKey, 'cx' => $googleCxId, 'q' => $query, 'searchType' => 'image', 'num' => 10, 'start' => $startIndex ];
...
elseif ($action === 'download' && isset($_GET['url'])) {
    // ... (Download/Resize logic - No changes needed here) ...
...
<body>
    <div class="container">
        <h1 class="my-4 text-center">h7 finder</h1>
    ...
    <div class="modal-overlay" id="image-preview-modal"> <div class="image-preview-content"> <button class="modal-close-button" aria-label="Close modal">&times;</button> <img alt="Image Preview" id="preview-image"> <div id="preview-caption" class="preview-caption"></div> </div> </div>
